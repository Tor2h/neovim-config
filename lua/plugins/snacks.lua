...
-- Ensure stdpath("cache") exists; snacks.nvim's Windows sqlite3.dll downloader
-- moves the DLL there but doesn't create the directory itself, which makes
-- Move-Item fail with "Could not find a part of the path."
...
map("n", "<leader>hn", "<cmd>lua require('snacks.picker').notifications()<CR>", { desc = "Notification History" })
map("n", "<leader>r", "<cmd>lua require('snacks').explorer()<CR>", { desc = "File Explorer" })
