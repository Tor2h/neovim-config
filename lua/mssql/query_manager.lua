...
local function as_error_text(value)
    ...
    return {
      connect_async = function(connect_params)
        ...
        if err then
          ...
          error("Could not connect: " .. as_error_text(err.message), 0)
        end

        result, err = utils.wait_for_notification_async(bufnr, client, "connection/complete", 10000)
        if err then
          state.set_state(states.Disconnected)
          error("Error in connecting: " .. as_error_text(err.message), 0)
        elseif result and has_value(result.errorMessage) then
          state.set_state(states.Disconnected)
          error("Error in connecting: " .. as_error_text(result.errorMessage), 0)
        end

        local intellisense_result
        intellisense_result, err = utils.wait_for_notification_async(bufnr, client, "textDocument/intelliSenseReady", 30000)
        if err then
          state.set_state(states.Disconnected)
          error("Intellisense failed to start: " .. as_error_text(err.message), 0)
        elseif intellisense_result and has_value(intellisense_result.errorMessage) then
          state.set_state(states.Disconnected)
          error("Error in connecting: " .. as_error_text(intellisense_result.errorMessage), 0)
        end

        if result and has_value(result.connectionSummary) then
      ...
      execute_async = function(query)
        ...
        local execute_err
        execute_result, execute_err = utils.lsp_request_async(client, "query/executeString", {
          query = query,
          ownerUri = owner_uri,
        })
        ...
        if execute_err then
          state.set_state(states.Connected)
          error("Error executing query: " .. as_error_text(execute_err.message), 0)
        elseif not execute_result then
          state.set_state(states.Connected)
